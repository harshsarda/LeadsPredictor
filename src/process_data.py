...
class DataProcessing:
    def __init__(self, fe, ohe, catnumagg, model_cols, logger):
        """
        FreqEnc object
        OHE object
        CatNumAgg object
    ...
    def run(self, data_dict):
        ...
        self.logger.info("data shape after ohe enc: {}".format(df.shape[1]))
        # perform cat num aggregations
        df = self.catnumagg.transform(df)
        self.logger.info("data shape after catnumagg fc: {}".format(df.shape[1]))
        # subset model cols
